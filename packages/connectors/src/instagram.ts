import type { SocialConnector, Connection, OAuthStartInput, OAuthCallbackInput, TokenSet, ProviderIdentity, PermissionManifest, Provider, PublishInput, PublishResult } from '@orh/shared'
...
export class InstagramConnector implements SocialConnector {
  ...
  async getIdentity(connection: Connection): Promise<ProviderIdentity> {
    ...
    const res = await fetch(`${IG_GRAPH_URL}/me?fields=id,username&access_token=${token}`)
