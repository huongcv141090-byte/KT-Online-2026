import type { SocialConnector, Connection, OAuthStartInput, OAuthCallbackInput, TokenSet, ProviderIdentity, PermissionManifest, Provider } from '@orh/shared'
...
export class MetaConnector implements SocialConnector {
  ...
  async revoke(connection: Connection): Promise<void> {
    const token = decrypt(connection.encryptedAccessToken)
    await fetch(
      `https://graph.facebook.com/${connection.providerUserId}/permissions?access_token=${token}`,
      { method: 'DELETE' },
    )
  }
}
